...
**Status**: Analysis Complete — Validated Against SDK RC4 (March 15, 2026)  
**Date**: 2026-03-02 (updated 2026-03-15)  
...
> **Note (March 15, 2026):** This spec was validated against the latest SDK source at [microsoft/agent-framework](https://github.com/microsoft/agent-framework) (renamed from `microsoft/Agents-for-net`). The `Microsoft.Agents.AI.Hosting` package is at `1.0.0-preview.260311.1`. All technical claims in this document remain accurate. The `AgentSessionStore` contract is stable (2 methods, unchanged since the spec was written).
>
> **Current v0.3 workaround:** Due to a breaking change in SDK RC4 where `GetService<InMemoryChatHistoryProvider>()` returns null after deserialization, messages are currently serialized independently in `AgentSessionData.SerializedMessages`. This workaround will be removed when we adopt the SDK’s `AgentSessionStore` contract with `StateBag`-based message access via `TryGetInMemoryChatHistory()` / `SetInMemoryChatHistory()`. See [Journal 03 — The Ghost in the Session](../../journal/03-%20The%20Ghost%20in%20the%20Session.md) for the full story.
