...
graph TB
    ...
    McpContainer -->|"AIAgent (Foundry)"| Foundry
...
Evolved from v0.3. Minimal changes: added `/health` endpoint awareness. The core class structure is unchanged — Docker support is entirely an infrastructure concern.
...
Unchanged from v0.3. The Foundry agent runs in the cloud and is accessed via HTTP regardless of whether the MCP server is containerized or running on the host.
...
| 13 | Class Diagram | Unchanged | Minimal changes (health endpoint only) |
| 14 | Stock Agent — Foundry Integration | Unchanged | Same regardless of deployment mode |
