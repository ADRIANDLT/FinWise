...
## Style & Architecture
- Agents and Workflow: C# (maybe adding Python Agents in the future) using Microsoft Agent framework SDK for .NET and Python, with MCP protocol for cross-agent communication.
- MCP servers: C# (and Python in the future) using MCP SDKs.
- Cross-language communication: MCP stdio (local) and HTTP+SSL (containerized/cloud).
