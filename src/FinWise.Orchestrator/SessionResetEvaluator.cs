...
/// </summary>
internal static class SessionResetEvaluator
{
    /// <summary>
    ...
    /// </summary>
    public static bool ShouldResetSession(IReadOnlyList<ChatMessage> conversationHistory, string? query)
    {
        ...
        if (!HasProfileReady(conversationHistory))
        {
            return false;
        }
    ...
    private static bool HasProfileReady(IReadOnlyList<ChatMessage> history)
    {
        return history.Any(m => m.Role == ChatRole.Assistant &&
            ...
            m.Text.Contains("PROFILE_READY:", StringComparison.OrdinalIgnoreCase));
    }
