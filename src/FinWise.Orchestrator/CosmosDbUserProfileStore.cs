...
public class CosmosDbUserProfileStore : IUserProfileStore, IAsyncDisposable
{
    ...
    /// <inheritdoc/>
    public async Task SetProfileAsync(string userId, UserProfileDto profile)
    {
        try
        {
            var container = await GetContainerAsync();
            ...
            // Merge: only update fields that are non-null in the incoming DTO
            var document = UserProfileDocument.FromDtoWithMerge(profile, existingDocument);
