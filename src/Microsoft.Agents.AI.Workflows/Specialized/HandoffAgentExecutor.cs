    ...
    string? handoffInstructions) : Executor(agent.GetDescriptiveId(), declareCrossRunShareable: true), IResettableExecutor
{
    ...
    private static string GetToolSafeAgentId(string agentId)
    {
        ...
        // Tool/function names must be identifier-like. Normalize anything outside [A-Za-z0-9_] to '_'.
        Span<char> buffer = stackalloc char[agentId.Length];
        int length = 0;
        foreach (char ch in agentId)
        {
            buffer[length++] = char.IsLetterOrDigit(ch) || ch == '_' ? ch : '_';
        }
    ...
    public void Initialize(
        ...
        builder.AddSwitch(this, sb =>
        {
            if (handoffs.Count != 0)
            {
                ...
                {
                    string targetId = GetToolSafeAgentId(handoff.Target.Id);
                    var handoffFunc = AIFunctionFactory.CreateDeclaration(
                        $"{HandoffsWorkflowBuilder.FunctionPrefix}{targetId}",
                        handoff.Reason,
                        s_handoffSchema);
